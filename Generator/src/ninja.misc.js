...
ninja.tab = {
    ...
    whichIsOpen: function () {
        var isOpen;
        for (var wType in ninja.wallets) {
            isOpen = false; //ninja.wallets[wType].isOpen();
            if (isOpen) {
                return wType;
            }
        }
...
ninja.generateKeys = function () {
	// TODO: hide button and show spinner
      //var sKeySize = $('#key-size').attr('data-value');
      var keySize = 4096; //parseInt(sKeySize);
      var crypt = new JSEncrypt({default_key_size: keySize});
      var dt = new Date();
      var time = -(dt.getTime());
	  // TODO : replace to avoid jquery
	document.getElementById("time-report").innerHTML = '*'
	//$('#time-report').text('.');
	var load = setInterval(function () {
	  //var text = $('#time-report').text();
	  var text = document.getElementById("time-report").innerHTML;
	  //$('#time-report').text(text + '.');
	  document.getElementById("time-report").innerHTML = text+' *';
	}, 500);
	crypt.getKey(function () {
	  ...
	  document.getElementById("time-report").innerHTML = 'Generated in ' + time + ' ms'
	  ...
	  var address = Crypto.SHA224(crypt.getPublicKey()); // , { asBytes: true }
	  // TODO: address
	  document.getElementById("bisaddress").value = address;
	});
	return;
};
